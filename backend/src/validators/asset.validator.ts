...
export const createAssetSchema = z.object({
  name: z.string({ message: 'Asset name is required' }).min(1).max(100),
  tag: z.string({ message: 'Asset tag is required' }).min(1).max(50).transform(val => val.toUpperCase()),
  serialNumber: z.string({ message: 'Serial number is required' }).min(1).max(100),
